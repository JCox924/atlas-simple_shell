...
### Shell Loop: The program enters a while loop that continues indefinitely (while(1)).

### Interactive Mode: Inside the loop, if the shell is running interactively, it prints a prompt (PROMPT) and flushes the standard output.
...
Inside the loop, it prints the shell prompt (PROMPT) and reads input from the user using theread_line() function. The read_line() function dynamically allocates memory to store the input line using getline().
